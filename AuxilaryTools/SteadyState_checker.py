...
for idec in range(len(decades)):
  # model output file
  fn1_raw=igcm_out+'/'+expid+which_run+'/SBG/Output/MO/'+expid+which_run+'_'+str(decades[idec])+'0101_'+str(decades[idec]+9)+'1231_1M_stomate_history.nc'
  ...
  for varname in varnames: 
    if varname=='Cveg': 
      ...
      tmp2=tmp
    if varname=='Ctot': 
      tmpa=ncres1.variables['SOIL_SOMA_c'][:]+ncres1.variables['SOIL_SOMC_c'][:]+ncres1.variables['SOIL_SOMP_c'][:]
      tmpb=ncres1.variables['MICROBE_R_AB_c'][:]+ncres1.variables['MICROBE_K_AB_c'][:]+ncres1.variables['MICROBE_R_BE_c'][:]+ncres1.variables['MICROBE_K_BE_c'][:]
      tmpc=ncres1.variables['TOTAL_M_c'][:]
      tmpd=ncres1.variables['LITTER_STR_AB_c'][:]+ncres1.variables['LITTER_STR_BE_c'][:]+ncres1.variables['LITTER_MET_AB_c'][:]+ncres1.variables['LITTER_MET_BE_c'][:]+ncres1.variables['LITTER_WOD_AB_c'][:]+ncres1.variables['LITTER_WOD_BE_c'][:]
      tmp = tmpa + tmpb +tmpc + tmpd
      tmp2=tmp

    if varname=='Ctot2': 
      tmpa=ncres1.variables['CARBON_PASSIVE'][:]+ncres1.variables['CARBON_ACTIVE'][:]+ncres1.variables['CARBON_SLOW'][:]
      tmpc=ncres1.variables['TOTAL_M'][:]
      tmpd=ncres1.variables['LITTER_STR_AB'][:]+ncres1.variables['LITTER_STR_BE'][:]+ncres1.variables['LITTER_MET_AB'][:]+ncres1.variables['LITTER_MET_BE'][:]
      tmp = tmpa +tmpc + tmpd
      tmp2=tmp
...
check('simulation years: '+str(decades[0])+'-'+str(decades[len(decades)-1]+9),logfile)
...
# Loop over the latitude and longitude dimensions
  for i in range(nlat):
    for j in range(nlon):
        ...
        if time_series_compressed.size > 0:
           slope, intercept, r_value, p_value, std_err = linregress(np.arange(ntime), time_series_compressed)
           trend[i, j] = slope
  ...
  if np.all(trend == missval):
      print((varname+' for PFT'+str(pft+1)+': no pixels with coverage above:'+str(cov_thres)))
      total=0.
      za=0.
  else:
     masked_trend = np.ma.masked_array(trend, mask=np.where(trend== missval, True, False)) 

     p31 = np.asarray(masked_trend[:,:])
     za = (abs(p31) < thr).sum()
     # count total pixels :
     total = np.ma.count(masked_trend)
     zr = za / float(total) *100.
     check(varname+' for PFT'+str(pft+1)+': percentage of pixel below threshold: '+str(round(zr,2))+'% of '+str(total)+' pixels', logfile)

  if pft > 0:
   A_total = A_total + total
   A_za    = A_za + za
  
 A_zr = A_za / float(A_total) *100.
 check(varname+' GLOBAL (excl PFT1): percentage of tiles below threshold: '+str(round(A_zr,2))+'% of '+str(A_total)+' tiles', logfile)
...
for varname in varnames: 
 for pft in range(npft):
  # output folder:
  file2create=trendy_out+'/ORCHIDEE-CNP_'+which_run+'_'+varname+'_PFT'+str(pft+1)+'.nc'
