...
def Cluster_Ana(packdata,auxil,PFT_mask,ipft,var_pred,var_pred_name,K,Nc):
  if 'Ndep_nhx_pft' in var_pred_name: packdata.Ndep_nhx_pft=packdata.Ndep_nhx[ipft-1]
  if 'Ndep_noy_pft' in var_pred_name: packdata.Ndep_noy_pft=packdata.Ndep_noy[ipft-1]
  if 'Pdep_pft' in var_pred_name: packdata.Pdep_pft=packdata.Pdep[ipft-1]
  pp=PFT_mask[ipft-1]#[:][:]
  ...
  for clus in range(K):
    ...
    All_selectedID=DataFrame(np.append(All_selectedID,SelectedID,axis=0),index=None,columns = ['lat','lon'])  
