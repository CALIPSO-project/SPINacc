...
def MLloop(packdata,auxil,ipool,logfile,varlist,labx,resultpath,fx,fy,fz):
  ...
  resp_list=[]
  for ii in ilist:
    resp_list+=[ii['name_prefix']+'_%2.2i'%jj+ii['name_postfix'] for jj in ii['pools']]
...
#        print(var_resp_biomass[indy],ipft)
        exec('pool_arr,pool_map=extract_Y.extract(responseY,auxil,var_resp_%s[indy],ipft)'%(ipool))
