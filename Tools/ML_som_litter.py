...
def MLloop(packdata, auxil, ipool, logfile, varlist, labx, resultpath, fx, fy, fz):
    ...
    for var_key in Yvar.keys():
        ...
        for ii in ilist:
            resp_list += [
                ii["name_prefix"] + "_%2.2i" % jj + ii["name_postfix"]
                for jj in ii["pfts"]
            ]
    ...
    for ipft in range(len(auxil.pfts)):
        check.display(
            "processing %s, PFT %2.2i..." % (ipool, auxil.pfts[ipft]), logfile
        )
        # extract data
        extr_var = extract_X.var(packdata, auxil, auxil.pfts[ipft])
        # extract PFT map
        pft_ny = extract_X.pft(packdata, auxil, PFT_mask_lai, auxil.pfts[ipft]).reshape(
            len(auxil.Nlat), 1
        )
        ...
        if len(adict["var_resp_%s" % ipool]) == len(auxil.pfts):
            ...
            cnp = [
                ipft,
                ipft + len(auxil.pfts),
                ipft + len(auxil.pfts) * 2,
            ]  # SOC,N and P
        for indy in cnp:
            check.display(
                "processing %s, PFT %2.2i, CNP %2.2i..."
                % (ipool, auxil.pfts[ipft], cnp.index(indy)),
                logfile,
            )
            nsubp = responseY[adict["var_resp_%s" % ipool][indy]].shape[1]
            check.display(
                "%s, PFT %2.2i, CNP %2.2i, has %2.2i subpools:"
                % (ipool, auxil.pfts[ipft], cnp.index(indy), nsubp),
                logfile,
            )
            for isubp in range(
                nsubp
            ):  # loop for 4 soil pools (surface,labile,passive,slow)
                check.display(
                ...
                lc = locals()
                exec(
                    "pool_arr,pool_map=extract_Y.extract(responseY,auxil,var_resp_%s[indy],isubp)"
                    % (ipool)
                )
                ...
                if (PFT_mask[auxil.pfts[ipft] - 1] > 0).any():
                    ...
                    check.display(
                        "%s PFT%2.2i element%i subp%i: R2=%.3f , RMSE=%.2f, slope=%.2f, reMSE=%.2f"
                        % (
                            ipool,
                            auxil.pfts[ipft],
                            cnp.index(indy),
