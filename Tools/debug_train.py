...
#                               max_depth=14, min_samples_split=5)
  ...
  ytests = []
  ypreds = []
  for train_idx, test_idx in loo.split(Xtrain):
    ...
    ytests += list(y_test)
    ypreds += list(y_pred)
  loocv_R2 = metrics.r2_score(ytests,ypreds)
  loocv_MSE = metrics.mean_squared_error(ytests, ypreds)   
  loocv_reMSE=len(ytests)*MSE/np.sum((ypreds-np.mean(ytests)**2))
  loocv_slope, intercept, r_value, p_value, std_err = stats.linregress(ytests,ypreds) 
  return TreeEns,predY,loocv_R2,loocv_reMSE,loocv_slope
