...
##@param[in]   subLabel             The names of sub-level iterm: nutrients or soil depth
...
'forestgreen','limegreen','gold','tomato','maroon']
    ...
    # slope
        ...
        for jj in range(0,subps):
            for ii in range(0,npfts):
                axs[1].text(-0.5+jj, ii,str(slope_n[ii,jj]), size = fonts, color='k',weight='bold')
        my_x_ticks = np.arange(subps)
...
# remse
        ...
        for jj in range(0,subps):
            for ii in range(0,npfts):
                axs[2].text(-0.5+jj, ii,str(dNRMSE_n[ii,jj]), size = fonts, color='k',weight='bold')
        my_x_ticks = np.arange(subps)
