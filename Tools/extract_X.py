...
def var(packdata,auxil,ipft):
  ...
  for indx in range(auxil.Nv_total):
    if indx<auxil.Nv_nopft:
      extracted_var=np.reshape(extract_X(packdata,auxil,indx,auxil.var_pred_name),(len(auxil.Nlat),1))
      extr_var=np.concatenate((extr_var,extracted_var),axis=1)
    else:
      extracted_var=np.reshape(extract_XN(packdata,auxil,indx,auxil.var_pred_name,ipft),(len(auxil.Nlat),1))
      extr_var=np.concatenate((extr_var,extracted_var),axis=1)
  return extr_var
