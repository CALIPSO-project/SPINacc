...
def readvar(varlist, config, logfile):
    ...
    for index in range(len(varname_clim)):
        # Initialize storage for raw 6-hourly data
        var_6hourly_year = []
        for year in range(climvar["year_start"], climvar["year_end"] + 1):
            ...
            # Handle compressed netCDF files
            ...
            var_6hourly_year.append(da)

        # Combine yearly data into a single array
        var_6hourly_year = np.concatenate(var_6hourly_year, axis=0)
        adict["Y%s" % varname_clim[index]] = var_6hourly_year

    # Store the raw 6-hourly data in packdata
    for var in varname_clim:
        packdata[var] = (["time", "lat", "lon"], adict["Y%s" % var])

    # TODO: replace the stats with full data for NN training
    # Example for Tair, converting from Kelvin to Celsius
    if "Tair" in adict:
        packdata.Tair = (["time", "lat", "lon"], adict["YTair"] - 273.15)

    # Read other variables, including Edaphic variables, N and P deposition variables
